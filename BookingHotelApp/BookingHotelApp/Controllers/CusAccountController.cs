...
namespace BookingHotelApp.Controllers
{
    ...
    [ApiController]
    public class CusAccountController : ControllerBase
    {
        //Khởi tạo đối tượng service
        private readonly CusAccountSvc _svc;
        public CusAccountController()
        {
            _svc = new CusAccountSvc();
        }
        [HttpGet("had-login")]
        public IActionResult HadLogin()
        {
            ...
            String hadLogin = HttpContext.Session.GetString("CusLoginSession"); 
        ...
        public IActionResult ValidateUser([FromBody]UserReq req)
        {
            ...
            if (result.Success)
            {
                HttpContext.Session.SetString("CusLoginSession", req.Username);
            }
            return Ok(result);
        }
    }
